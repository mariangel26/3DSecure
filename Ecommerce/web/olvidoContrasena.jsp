...
<!DOCTYPE html>
<html>
    <title>Olvidar Contrasena</title>
 <link href="css/style.css" rel="stylesheet" type="text/css"/>
 <%
            String user; 
            user = request.getParameter("user");
            String pregunta = new DAOCliente().buscarCuenta(user).getPreguntaSecreta();
        %>
        <form class='login-form' action="olvidoContrasena" method="post" onsubmit="return validar()">
        
        <input type="hidden" value="<%=user%>" name="usuario">
   <center>
         <a>
             <th class='lf--signup' align='center'>Pregunta: <%= pregunta%></th>
        </a>
        </center>
  ...
  <div class="flex-row">
    ...
    <input id="txtrespuesta" class='lf--input' placeholder='Respuesta Secreta' type='text' name="respuesta">
  </div>
  ...
  <div class="flex-row">
    ...
    <input id="txtpassword" class='lf--input' placeholder='Password' type='password' name="contrasena">   
  </div>
  <div class="flex-row">
    <label class="lf--label" for="password">
      <svg x="0px" y="0px" width="15px" height="5px">
    ...
    <input id="txtpassword2" class='lf--input' placeholder='repetir Password' type='password' name="contrasena2">   
  </div>
  ...
  <input class='lf--submit' type='submit' value='CAMBIAR CONTRASEÑA'>
</form>
  <script>
      function validar(){
    ...
    function noHayVacio(){
        ...
        if(document.getElementById("txtpassword2").value===""){
            alert("EL CAMPO DE CONTRASENA 2 NO PUEDE ESTAR VACIO");
            respuesta = false;
        }
